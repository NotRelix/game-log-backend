...
export const getPostHandler = factory.createHandlers(async (c) => {
    try {
        ...
        return c.json({ success: false, messages: ["Failed to get single post"] }, 500);
    }
});
...
export const deletePostHandler = factory.createHandlers(async (c) => {
    try {
        ...
        return c.json({ success: true, messages: ["Successfully deleted post"], post: deleted }, 200);
    }
    catch (err) {
