...
export const getCommentsHandler = factory.createHandlers(async (c) => {
    try {
        ...
        return c.json({ success: false, messages: ["Failed to get comments"] }, 500);
    }
});
export const createCommentHandler = factory.createHandlers(validator(commentSchema), async (c) => {
    try {
        ...
        const createdComment = await createCommentDb(body.comment, postId, user.id);
        return c.json({
            success: true,
        ...
        return c.json({ success: false, messages: ["Failed to create a comment"] }, 500);
    }
});
export const editCommentHandler = factory.createHandlers(validator(commentSchema), async (c) => {
    try {
        const postId = Number(c.req.param("postId"));
        if (isNaN(postId)) {
            return c.json({ success: false, messages: ["Invalid post id"] }, 400);
        ...
        if (!comment) {
            return c.json({ success: false, messages: ["Comment doesn't exist"] }, 404);
        }
        const user = c.get("jwtPayload");
        ...
        return c.json({ success: false, messages: ["Failed to edit comment"] }, 500);
    }
});
export const deleteCommentHandler = factory.createHandlers(async (c) => {
    try {
        ...
        return c.json({ success: false, messages: ["Failed to delete comment"] }, 500);
    }
});
